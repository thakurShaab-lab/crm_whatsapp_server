...
async function sendViaGupshup({ employee, mobile, type, text, media }) {
  ...
  if (type !== 'text') {
    ...
    const caption = `Dear Sir,%0APlease check this ${GUPSHUP_FILE_TYPE_LABEL[type]}.`
    requestUrl =
      ...
      `&caption=${encodeURIComponent(caption)}&media_url=${encodedMediaUrl}`
  } else {
    const url = new URL('https://mediaapi.smsgupshup.com/GatewayAPI/rest')
...
function buildAiSensyMediaPayload({ mobile, type, media }) {
  const attachmentType = AISENSY_NETCORE_ATTACHMENT_TYPE[type]
  const link = requirePublicMediaUrl(media.url)
  if (type === 'document') {
    return { to: mobile, type: attachmentType, [attachmentType]: { caption: media.originalFilename, link, filename: 'wds' } }
  }
  if (type === 'audio') {
    return { to: mobile, type: attachmentType, [attachmentType]: { link } }
  }
  return { to: mobile, type: attachmentType, [attachmentType]: { caption: media.originalFilename, link } }
}

async function sendViaAiSensy({ employee, mobile, type, text, media }) {
  const freshEmployee = await refreshAiSensyTokenIfNeeded(employee)

  const payload =
    type === 'text' ? { to: mobile, type: 'text', text: { body: text } } : buildAiSensyMediaPayload({ mobile, type, media })
...
function buildNetcoreBody({ mobile, type, text, media }) {
  return {
    message: [
      {
                  ...
                  attachments: [
                    {
                      ...
                      caption: media.originalFilename,
...
async function sendViaNetcore({ employee, mobile, type, text, media }) {
  ...
  const response = await fetch(endpoint, {
    ...
    body: JSON.stringify(buildNetcoreBody({ mobile, type, text, media })),
  })
  const data = await response.json()
 ...
 */
export async function sendMessage({ employee, mobile, type, text, media }) {
  if (employee.whatsappVendor === 'A') return sendViaAiSensy({ employee, mobile, type, text, media })
  if (employee.whatsappVendor === 'C') return sendViaNetcore({ employee, mobile, type, text, media })
  return sendViaGupshup({ employee, mobile, type, text, media })
}
